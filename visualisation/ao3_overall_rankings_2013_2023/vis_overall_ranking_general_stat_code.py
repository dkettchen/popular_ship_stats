...
def fic_type_by_gender_combo(input_df:pd.DataFrame):
    ...
    returns a dict with series values containing the number of gen and slash pairings for 
    each gender combo that year
    """
    new_df = input_df.copy()
    ...
    for year in unique_year_list:
        ...
        year_df = rename_gender_combos(year_df, column=True) # this should rename the labels first
        year_df = get_label_counts(year_df, ["fic_type", "gender_combo"], "ship") # then counts them
