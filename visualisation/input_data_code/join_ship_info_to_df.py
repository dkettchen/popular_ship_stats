...
def join_ship_info_to_df(input_df_dict:dict, ranking:str):
    ...
    # setting base df & ship name
    if ranking == "femslash":
        ship_name = "slash_ship"
        ships_df = base_ships_df
    elif ranking in ["overall", "annual"]:
        ship_name = "ship"

        # making two dfs, one w only slash ship tags & one w only gen ship tags
        slash_df = base_ships_df.copy().rename(columns={"slash_ship":ship_name})
        slash_df.pop("gen_ship")
        gen_df = base_ships_df.copy().rename(columns={"gen_ship":ship_name})
